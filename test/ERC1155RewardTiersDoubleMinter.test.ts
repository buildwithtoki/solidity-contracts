...
import type { ERC1155RewardTiersDoubleMinter, RewardTierERC1155 } from "../typechain-types/contracts";
import type { INativeMinter } from "../typechain-types/contracts/precompiles";
import { parseRoleNumber, parseToki } from "../util";
import { promisify } from "util";
import rewardTierInfo from "../rewardTierErc1155DeployConfig.json";
...
async function initializeRewardTierERC1155Token(admin: SignerWithAddress, creator: SignerWithAddress) {
  const tokenFactory = await ethers.getContractFactory("RewardTierERC1155", admin);
  return tokenFactory.deploy(
    creator.address,
    rewardTierInfo.maxTotalSupplies,
    rewardTierInfo.rewardTierNames,
    rewardTierInfo.requiredRewardAmounts,
    rewardTierInfo.uris
  );
}

if (hre.network.name !== "hardhat") {
  describe("ERC1155RewardTiersDoubleMinter", () => {
    let admin: SignerWithAddress,
      deployer: SignerWithAddress,
      normalUser: SignerWithAddress;

    let nativeToken: INativeMinter;
    let erc1155Token: RewardTierERC1155;
    ...
    before(async () => {
      ...
      erc1155Token = await initializeRewardTierERC1155Token(admin, admin) as RewardTierERC1155;
    });
    ...
    it("can atomically drop ERC1155 and native tokens in one transaction", async () => {
      ...
      const erc1155DoubleMinterFactory = await ethers.getContractFactory("ERC1155RewardTiersDoubleMinter", admin);
      const erc1155DoubleMinter = await erc1155DoubleMinterFactory.deploy(erc1155Token.address) as ERC1155RewardTiersDoubleMinter;
...

      {
        ...
        const role = await erc1155Token.readAllowList(admin.address);
        console.log(`Admin role: ${ parseRoleNumber(role) }`);
      }

      const transactions = [];

      console.log("Enabling ERC1155RewardTiersDoubleMinter for Toki native coin");
      transactions.push(await nativeMinterContract.connect(admin).setEnabled(erc1155DoubleMinter.address));

      console.log(`Enabling ERC1155RewardTiersDoubleMinter on ERC1155...`);
      transactions.push(await erc1155Token.connect(admin).setEnabled(erc1155DoubleMinter.address));
      ...
      transactions.push(await erc1155Token.connect(admin).setEnabled(deployer.address));
      ...
      {
        const doubleRole = await erc1155Token.readAllowList(erc1155DoubleMinter.address);
        const deployerRole = await erc1155Token.readAllowList(deployer.address);
        expect(deployerRole).to.be.equal(BigNumber.from(1));
        expect(doubleRole).to.be.equal(BigNumber.from(1));
      }
    ...
    //                   ERC1155  Native
    // ERC1155RewardTiersDoubleMinterRole:   1       1
    // ERC1155RewardTiersDoubleMinter:       1       1
    // Token Owner:        1       0
    describe("can mint native token as a proxy for token key", () => {
      let erc1155DoubleMinter: ERC1155RewardTiersDoubleMinter;
      ...
      before(async () => {
        ...
        const erc1155DoubleMinterFactory = await ethers.getContractFactory("ERC1155RewardTiersDoubleMinter", admin);
        erc1155DoubleMinter = await erc1155DoubleMinterFactory.deploy(erc1155Token.address) as ERC1155RewardTiersDoubleMinter;

        console.log("Setting permissions...");
        const transactions = [];

        transactions.push(await erc1155Token.setEnabled(erc1155DoubleMinter.address));
        transactions.push(await nativeToken.setEnabled(erc1155DoubleMinter.address));

        transactions.push(await erc1155Token.setEnabled(tokenOwner.address));
        ...
        transactions.push(await erc1155Token.setEnabled(erc1155DoubleMinterRole.address));
      ...
      it("Token owner is admin for token", async () => {
        const role = await erc1155Token.readAllowList(tokenOwner.address);
      ...
      it("ERC1155RewardTiersDoubleMinterRole owns ERC1155RewardTiersDoubleMinter", async () => {
        const owner = await erc1155DoubleMinter.owner();
        expect(owner).to.equal(erc1155DoubleMinterRole.address);
      });

      it("ERC1155RewardTiersDoubleMinter Contract enabled for ERC1155", async () => {
        const role = await erc1155Token.readAllowList(erc1155DoubleMinter.address);
        expect(role).to.be.equal(BigNumber.from(1));
      });

      it("ERC1155RewardTiersDoubleMinter Contract enabled for native", async () => {
        const role = await nativeToken.readAllowList(erc1155DoubleMinter.address);
        expect(role).to.be.equal(BigNumber.from(1));
      });

      it("ERC1155RewardTiersDoubleMinter Role enabled for ERC1155", async () => {
        const role = await erc1155Token.readAllowList(erc1155DoubleMinterRole.address);
        expect(role).to.be.equal(BigNumber.from(1));
      });

      it("ERC1155RewardTiersDoubleMinter Role enabled for native", async () => {
      ...
      describe("Token owner can NOT mint native tokens via", () => {
        it("fails calling ERC1155RewardTiersDoubleMinter contract mintTokens()", async () => {
          await expect((async () => {
      ...
      it("ERC1155RewardTiersDoubleMinterRole can drop tokens with ERC1155RewardTiersDoubleMinter Contract", async () => {
        const beforeBalance = {
          native: await ethers.provider.getBalance(normalUser.address),
          erc1155: await erc1155Token.balanceOf(normalUser.address, 1),
        };
