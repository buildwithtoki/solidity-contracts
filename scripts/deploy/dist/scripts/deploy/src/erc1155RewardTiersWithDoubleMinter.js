...
import { deployERC1155RewardTierWithAWS } from "./utils/deployTokenWithAWS.js";
...
if (!process.env.TOKEN_CREATOR_ADDRESS) {
    console.error("Please set environment variable: TOKEN_CREATOR_ADDRESS");
    process.exit(1);
}
if (!process.env.TOKEN_MAX_TOTAL_SUPPLIES) {
    console.error("Please set environment variable: TOKEN_MAX_TOTAL_SUPPLIES");
    process.exit(1);
}
if (!process.env.TOKEN_REWARD_TIER_NAMES) {
    console.error("Please set environment variable: TOKEN_REWARD_TIER_NAMES");
    process.exit(1);
}
if (!process.env.TOKEN_REQUIRED_REWARD_AMOUNTS) {
    console.error("Please set environment variable: TOKEN_REQUIRED_REWARD_AMOUNTS");
    process.exit(1);
}
if (!process.env.TOKEN_URIS) {
    console.error("Please set environment variable: TOKEN_URIS");
    process.exit(1);
}
const { TOKEN_IDENTIFIER: identifier, TOKEN_CREATOR_ADDRESS: creatorAddress, } = process.env;
if (!ethers.utils.isAddress(creatorAddress.trim())) {
    console.error("The provided TOKEN_CREATOR_ADDRESS is invalid.");
    process.exit(1);
}
const sMaxTotalSupplies = process.env.TOKEN_MAX_TOTAL_SUPPLIES.split(",");
const rewardTierNames = process.env.TOKEN_REWARD_TIER_NAMES.split(",");
const sRequiredRewardAmounts = process.env.TOKEN_REQUIRED_REWARD_AMOUNTS.split(",");
const uris = process.env.TOKEN_URIS.split(",");
if (sMaxTotalSupplies.length !== rewardTierNames.length || sMaxTotalSupplies.length !== sRequiredRewardAmounts.length || sMaxTotalSupplies.length !== uris.length) {
    console.error("The provided environment variables were not parsed correctly or were invalid.");
    process.exit(1);
}
const maxTotalSupplies = Array.from(sMaxTotalSupplies, (x, t) => Number(t));
rewardTierNames.forEach(t => t.trim());
const requiredRewardAmounts = Array.from(sRequiredRewardAmounts, (x, t) => Number(t));
uris.forEach(t => t.trim());
...
try {
    ...
    console.info(`  Creator Address:            ${creatorAddress}`);
    console.info(`  Max Total Supplies:         ${maxTotalSupplies.toString()}`);
    console.info(`  Reward Tier Names:          ${rewardTierNames.toString()}`);
    console.info(`  Max Total Supplies:         ${maxTotalSupplies.toString()}`);
    const { tokenContract, tokenContractOwner: tokenRole, secretName, doubleMinterAddress, doubleMinterRole } = await deployERC1155RewardTierWithAWS({
        identifier,
        creatorAddress,
        maxTotalSupplies,
        rewardTierNames,
    ...
    console.info(`Deployed the reward tiers contract! Address: ${tokenContract.address}`);
    console.info();
    // configure double minter for this token
    console.info("Creating new DoubleMinter...");
    const doubleMinterContract = new ethers.Contract(doubleMinterAddress, ERC1155RewardTiersDoubleMinterBinary.abi, deployer);
    console.info(`  DoubleMinter: ${doubleMinterContract.address}`);
    console.info();
    console.info("Giving the DoubleMinter Role Reward Tier and TOKI minting access...");
    const doubleMinterNativeEnabled = await (await nativeMinter.setEnabled(doubleMinterRole.address)).wait();
    const doubleMinterERC1155Enabled = await (await tokenContract.setEnabled(doubleMinterRole.address)).wait();
    console.info(`  Native TOKI Minting Enabled: ${doubleMinterNativeEnabled.status}`);
    console.info(`  Reward Tier Minting Enabled: ${doubleMinterERC1155Enabled.status}`);
    console.info();
    console.info("Giving DoubleMinter contract Reward Tier and TOKI minting access...");
    ...
    console.info(`  Reward Tier  Minting Enabled: ${tokenContractEnabled.status}`);
    console.info();
    // give ownership of double minter to token role
    {
        ...
        console.info("Transferring ownership from deployer to token role...");
        ...
        console.info(`  Reward Tier Contract: New: ${newTokenOwner} Old: ${oldTokenOwner}`);
        console.info();
    }
