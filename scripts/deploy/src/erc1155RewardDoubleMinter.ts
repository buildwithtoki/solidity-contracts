...
try {
  ...
  const { tokenContract, collectionId, tokenContractOwner: tokenRole, secretName, doubleMinterAddress, doubleMinterRole }
  = await deployERC1155ActivityDoubleMinterWithAWS(
    {
      identifier,
      tokenAddress,
      uri,
  ...
  const doubleMinterContract = new ethers.Contract(doubleMinterAddress, ERC1155ActivitiesDoubleMinterBinary.abi, deployer) as ERC1155ActivitiesDoubleMinter;
