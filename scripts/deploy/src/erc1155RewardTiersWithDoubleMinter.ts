...
const {
  ...
  TOKEN_CREATOR_ADDRESS: creatorAddress,
} = process.env;
...
try {
  ...
  = await deployERC1155RewardTierWithAWS(
    {
      identifier,
      creatorAddress,
      maxTotalSupplies,
  ...
  const doubleMinterContract = new ethers.Contract(doubleMinterAddress, ERC1155RewardTiersDoubleMinterBinary.abi, deployer) as ERC1155RewardTiersDoubleMinter;
